...
totalprop_race <- ggplotly(ggplot(data = totalvictims_race,
                                  aes(x = Victim.s.race, y = proportion, fill = Victim.s.race)) +
                             geom_bar(stat = "identity") +
                             theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
                             labs(title = "Proportion of Victims in Police Killings by Race from 2013-2020"))
...
proportion_year_fn <- function(yearUsed) {
  prop_by_year <- victim_race_data %>%
    filter(year == yearUsed) %>%
    group_by(Victim.s.race) %>%
